...
/pairs/{pairId}/items/{itemId}       ← 主要インデックス設計対象
  ...
  completed_at(Timestamp|null), reminderAt(Timestamp|null),
...
| IDX-03 | `items` | リマインダー対象クエリ（機能3） | `status` ASC | `reminderAt` ASC | 不可・**手動必須** |
...
```json
{
  "indexes": [
    ...
    },
    {
      ...
      "fields": [
        { "fieldPath": "status", "order": "ASCENDING" },
        { "fieldPath": "reminderAt", "order": "ASCENDING" }
      ]
    }
  ],
  "fieldOverrides": []
...
| Q-03 | 機能3: リマインダー対象取得 | `items where status=="open" AND reminderAt <= now` | IDX-03 | 対象件数 |
...
| Q-10 | 機能3: スヌーズ更新 | `items/{itemId}` update (reminderAt = now + 15min) | なし | Write: 1 |
